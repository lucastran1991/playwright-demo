...
// TestTraceFull_LevelsCapped verifies ?levels=99 does not panic and returns a valid response.
func TestTraceFull_LevelsCapped(t *testing.T) {
	router, cleanup := setupTestRouter(t)
	defer cleanup()

	w, resp := doTraceRequest(t, router, "/api/trace/full/RACKPDU-01?levels=99")

	if w.Code != 200 {
		t.Fatalf("expected 200, got %d (levels=99 should be capped, not panic)", w.Code)
	}
	if resp.Data == nil {
...
func TestTraceFull_ResponseSchema(t *testing.T) {
	...
	for _, g := range resp.Data.Upstream {
		...
		for _, n := range g.Nodes {
			if n.NodeID == "" {
				t.Error("traced node node_id must not be empty")
			}
			if n.Name == "" {
				t.Error("traced node name must not be empty")
			}
			if n.NodeType == "" {
				t.Error("traced node node_type must not be empty")
			}
			if n.ID == 0 {
				t.Error("traced node id must not be zero")
			}
		}
	}
}

// TestTraceFull_EmptyUpstream verifies GEN-01 (top of chain) has an empty upstream.
func TestTraceFull_EmptyUpstream(t *testing.T) {
	router, cleanup := setupTestRouter(t)
	defer cleanup()

	w, resp := doTraceRequest(t, router, "/api/trace/full/GEN-01")
	...
	if resp.Data.Source.NodeID != "GEN-01" {
		t.Errorf("source node_id: want GEN-01, got %q", resp.Data.Source.NodeID)
	}
	if len(resp.Data.Upstream) != 0 {
...
// TestTraceFull_LoadSection_CapacityNode verifies RPP-01 (IsCapacityNode=true) has load with Rack.
func TestTraceFull_LoadSection_CapacityNode(t *testing.T) {
	...
	if len(resp.Data.Load) == 0 {
		t.Error("RPP-01 (IsCapacityNode=true) should have a non-empty load section")
	}
	if findNodeInLocalGroups(resp.Data.Load, "RACK-01") == nil {
		t.Error("expected RACK-01 in load section for RPP-01")
	}
}

// TestTraceFull_LoadSection_NonCapacityNode verifies RACKPDU-01 (IsCapacityNode=false) has empty load.
func TestTraceFull_LoadSection_NonCapacityNode(t *testing.T) {
	...
	if len(resp.Data.Load) != 0 {
		t.Errorf("RACKPDU-01 (IsCapacityNode=false) should have empty load, got %d groups", len(resp.Data.Load))
	}
}
